...
public class Map {
	...
	private Vertex[] createMap(String mapFile) {
		try {
			List<String> listVertAdded = new ArrayList<>();
			...
			while (!(line = br.readLine()).equals("-1")) {
				...
				if (!contains(tokens[0])) {
					...
					System.out.println("neu chua co vertex: " +map[i] + " = new Vertex("+tokens[0] +")");
					
					// add an edge to new vertex
					Vertex edge = getVertexWithName(tokens[1]);


					// if edge vertex doesn't exist in map yet, create the edge vertex and connect it to vertex
					if (edge == null) {
						// add edges to each vertex using second and third value from file
						Vertex newEdge = new Vertex(tokens[1]);
//						listVertAdded.add(newEdge.toString());
						System.out.println("Vertex " + newEdge + "= new vertex(" +tokens[1]+")");
						// add each vertex to map
						map[i].addNeighbour(new Edge(newEdge, Double.valueOf(tokens[2])));
						System.out.println(map[i]+".addneghbour(new Edge("+newEdge+"," + tokens[2]);

					} else {
						map[i].addNeighbour(new Edge(edge, Double.valueOf(tokens[2])));
						System.out.println(map[i]+".addneghbour(new Edge("+edge+"," + tokens[2]);
					}
					...
					if (edge == null) {
						...
						System.out.println(vertex+".addneghbour(new Edge("+newEdge+"," + tokens[2]);
					} else {
						vertex.addNeighbour(new Edge(edge, Double.valueOf(tokens[2])));
						System.out.println(vertex+".addneghbour(new Edge("+edge+"," + tokens[2]);
					}
				}
				if (i > vertices) {
